...
check_scheduled_tasks() {
    ...
    if command -v impacket-smbexec &>/dev/null || command -v impacket-wmiexec &>/dev/null; then
        ...
        if [ -f lolbins/scheduled_tasks.csv ] && [ -s lolbins/scheduled_tasks.csv ]; then
            ...
            grep -iE 'powershell|cmd|wscript|cscript|mshta|rundll32|regsvr32|certutil' lolbins/scheduled_tasks.csv > lolbins/suspicious_scheduled_tasks.txt 2>/dev/null
...
check_startup_items() {
    ...
    if [ -d "gpo/sysvol" ]; then
        # Group the -name tests with \( ... \); without it, -type f only binds to
        # the first -name (find precedence: -a before -o), so *.cmd/*.ps1
        # directories would also match.
        find gpo/sysvol -type f \( -name "*.bat" -o -name "*.cmd" -o -name "*.ps1" \) 2>/dev/null \
            | grep -i "startup\|logon" > lolbins/startup_script_files.txt 2>/dev/null
...
check_suspicious_services() {
    ...
    if [ "$AUTH_TYPE" == "userpass" ] && command -v impacket-wmiexec &>/dev/null; then
        ...
        if [ -f lolbins/services.txt ] && [ -s lolbins/services.txt ]; then
            ...
            grep -iE 'temp|appdata|public|downloads|users' lolbins/services.txt > lolbins/suspicious_service_paths.txt 2>/dev/null
...
check_wmi_persistence() {
    ...
    if [ "$AUTH_TYPE" == "userpass" ] && command -v impacket-wmiexec &>/dev/null; then
        ...
        # Query WMI event consumers (common persistence). The namespace must be
        # the LOCAL root\subscription (the leading "\\.\" = this machine);
        # "\\root\subscription" without the dot is read as a host named "root".
...
check_registry_autoruns() {
    ...
    if [ "$AUTH_TYPE" == "userpass" ] && command -v impacket-reg &>/dev/null; then
        ...
        if [ -f lolbins/registry_autoruns.txt ] && [ -s lolbins/registry_autoruns.txt ]; then
            ...
            grep -iE 'temp|appdata|public|powershell|cmd|wscript' lolbins/registry_autoruns.txt > lolbins/suspicious_autoruns.txt 2>/dev/null
...
check_dll_search_order() {
    ...
    if [ -f "shares/writable_shares.txt" ]; then
        ...
        grep -iE 'windows|system32|program files' shares/writable_shares.txt > persistence/dll_hijack_opportunities.txt 2>/dev/null
