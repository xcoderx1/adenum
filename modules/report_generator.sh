...
generate_reports() {
    ...
    # Rebuild severity counters from the findings file so the report's stat
    # boxes are accurate even when findings were appended inside subshell pipes
    # (where the in-memory counters would have been lost).
    ...
    # Structured exports (JSON / CSV / SARIF) from findings.jsonl, if python3
    # is available. Degrades gracefully when it is not.
    local jsonl="${FINDINGS_JSONL:-findings.jsonl}"
    if [ "${PYTHON_AVAILABLE:-false}" = true ] && [ -f "$jsonl" ] && [ -f "$SCRIPT_DIR/modules/report_export.py" ]; then
        if python3 "$SCRIPT_DIR/modules/report_export.py" "$jsonl" "reports" 2>/dev/null; then
            log_success "Structured exports → reports/findings.{json,csv,sarif}"
        fi
    fi
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-low { background: #38a169; color: white; }
        ...
        .finding-card.low { border-left-color: #38a169; background: #f0fff4; }
        ...
        .level-low { background: #38a169; color: white; }
...
HTMLHEADER
    ...
    # Add dynamic content (escape user-controlled values to prevent HTML/JS
    # injection into the report).
    echo "                Domain: $(escape_html "$DOMAIN") | DC: $(escape_html "$DC_IP") | Generated: $(escape_html "$(date)")" >> "$html_file"
    echo "            </p>" >> "$html_file"
    echo "        </div>" >> "$html_file"
    echo "        " >> "$html_file"
    ...
    cat >> "$html_file" << EOF
            ...
            <div class="stat-box">
                <div class="stat-number low">$LOW_FINDINGS</div>
        ...
        <div class="controls">
            ...
            <button class="filter-btn btn-low" onclick="filterByLevel('low')">Low</button>
