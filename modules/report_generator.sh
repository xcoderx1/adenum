...
EOF
    ...
    # Parse findings file and add to HTML
...
EOF
            ...
            if [ -n "$exploit_ref" ] && [ "$exploit_ref" != "" ]; then
...
EOFRECOMMEND
    ...
    find . -type f \( -name "*.ldif" -o -name "*.txt" -o -name "*.json" -o -name "*.zip" -o -name "*.log" \) | while read file; do
        echo "  • $file" >> "$text_file"
    done
