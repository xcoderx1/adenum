#!/bin/bash
################################################################################
# MODULE: LDAP Enumeration - FIXED VERSION
# Coverage: Basic AD objects, password policies, privileged accounts
# FIX: Extracts actual usernames/computers for findings
################################################################################
...
# NEW: Extract usernames from LDIF file
extract_usernames() {
    local ldif_file="$1"
    local output_file="${ldif_file%.ldif}_names.txt"
    
    grep "sAMAccountName:" "$ldif_file" | awk '{print $2}' | sort -u > "$output_file"
    echo "$output_file"
}

# NEW: Extract computer names from LDIF file
extract_computers() {
    local ldif_file="$1"
    local output_file="${ldif_file%.ldif}_computers.txt"
    
    grep -E "sAMAccountName:|dNSHostName:" "$ldif_file" | awk '{print $2}' | grep -v '^$' | sort -u > "$output_file"
    echo "$output_file"
}
...
run_ldap_enum() {
    ...
    if [ -f "ldap/04_krbtgt.ldif" ]; then
        ...
        if [ -n "$pwd_last_set" ] && [ "$pwd_last_set" != "0" ]; then
