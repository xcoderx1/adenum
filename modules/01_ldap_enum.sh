...
#
# run_ldap() and the LDIF helpers (ldif_values / ldif_count_dn) live in
# modules/00_common.sh, which is sourced first. This module focuses on the
# enumeration logic and finding generation.
################################################################################

# Extract sAMAccountName values from an LDIF file to a sidecar .txt file.
# Uses the folding- and base64-aware ldif_values helper so wrapped or encoded
# names are not lost. Echoes the path of the created file.
extract_usernames() {
    local ldif_file="$1"
    local output_file="${ldif_file%.ldif}_names.txt"

    ldif_values sAMAccountName "$ldif_file" | grep -v '^$' | sort -u > "$output_file"
    echo "$output_file"
}

# Extract computer identifiers (sAMAccountName + dNSHostName) from an LDIF file.
extract_computers() {
    local ldif_file="$1"
    local output_file="${ldif_file%.ldif}_computers.txt"

    { ldif_values sAMAccountName "$ldif_file"; ldif_values dNSHostName "$ldif_file"; } \
...
run_ldap_enum() {
    ...
    priv_count="${priv_count//[^0-9]/}"
    [ -z "$priv_count" ] && priv_count=0
    
    local protected_count=$(grep -c "^dn:" "ldap/03_protected_users.ldif" 2>/dev/null || echo 0)
    protected_count="${protected_count//[^0-9]/}"
    [ -z "$protected_count" ] && protected_count=0
    ...
    if [ "$priv_count" -gt 0 ] && [ "$protected_count" -lt "$priv_count" ]; then
        ...
        local priv_users=$(extract_usernames "ldap/03_all_privileged.ldif")
        local protected_users=$(extract_usernames "ldap/03_protected_users.ldif")
        
        # Find unprotected users
        local unprotected_list=$(comm -23 <(sort "$priv_users") <(sort "$protected_users") | head -10 | tr '\n' ',' | sed 's/,$//')
        
        add_finding "HIGH" "Privileged Accounts" \
            "$unprotected privileged accounts NOT in Protected Users group: $unprotected_list" \
    ...
    pwd_never_count="${pwd_never_count//[^0-9]/}"
    [ -z "$pwd_never_count" ] && pwd_never_count=0
    
    if [ "$pwd_never_count" -gt 0 ]; then
        # FIX: Extract actual usernames
        local pwd_never_users=$(extract_usernames "ldap/04_password_never_expires.ldif")
        local user_list=$(head -5 "$pwd_never_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "MEDIUM" "Password Policy" \
            "$pwd_never_count accounts with password never expires: $user_list" \
    ...
    pwd_notreq_count="${pwd_notreq_count//[^0-9]/}"
    [ -z "$pwd_notreq_count" ] && pwd_notreq_count=0
    
    if [ "$pwd_notreq_count" -gt 0 ]; then
        # FIX: Extract actual usernames
        local pwd_notreq_users=$(extract_usernames "ldap/04_password_not_required.ldif")
        local user_list=$(cat "$pwd_notreq_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "CRITICAL" "Password Policy" \
            "$pwd_notreq_count accounts can have EMPTY passwords: $user_list" \
            "Test each: smbclient -L //$DC_IP -U 'USERNAME%' (no password)"
    fi
    ...
    if [ -f "ldap/04_krbtgt.ldif" ]; then
        local pwd_last_set=$(ldif_values pwdLastSet "ldap/04_krbtgt.ldif" | head -1)
        # Require a non-zero, all-digit FILETIME before doing arithmetic on it,
        # so an empty/non-numeric value can never throw an arithmetic error.
        if [ -n "$pwd_last_set" ] && [ "$pwd_last_set" != "0" ] && [[ "$pwd_last_set" =~ ^[0-9]+$ ]]; then
            # Convert Windows FILETIME to Unix timestamp
            local unix_time=$(( (pwd_last_set / 10000000) - 11644473600 ))
            local current_time=$(date +%s)
            local days_old=$(( (current_time - unix_time) / 86400 ))
            
            if [ $days_old -gt 180 ]; then
                add_finding "HIGH" "KRBTGT Password" \
                    "KRBTGT password is $days_old days old (should be <180 days)" \
    ...
    sid_hist_count="${sid_hist_count//[^0-9]/}"
    [ -z "$sid_hist_count" ] && sid_hist_count=0
    
    if [ "$sid_hist_count" -gt 0 ]; then
        local sid_users=$(extract_usernames "ldap/05_sid_history.ldif")
        local user_list=$(head -5 "$sid_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "MEDIUM" "SID History" \
            "$sid_hist_count accounts with SID History (potential backdoor): $user_list" \
    ...
    if [ "$test_count" -gt 0 ]; then
        local test_users=$(extract_usernames "ldap/05_test_temp_accounts.ldif")
        local user_list=$(cat "$test_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "MEDIUM" "Account Hygiene" \
            "$test_count test/temp accounts found: $user_list" \
    ...
    rev_enc_count="${rev_enc_count//[^0-9]/}"
    [ -z "$rev_enc_count" ] && rev_enc_count=0
    
    if [ "$rev_enc_count" -gt 0 ]; then
        local rev_users=$(extract_usernames "ldap/05_reversible_encryption.ldif")
        local user_list=$(head -5 "$rev_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "HIGH" "Password Storage" \
            "$rev_enc_count accounts store passwords with reversible encryption: $user_list" \
    ...
    if [ -f "ldap/06_domain_config.ldif" ]; then
        ...
        if [ -n "$quota" ] && [ "$quota" -gt 0 ]; then
            add_finding "MEDIUM" "Machine Account Quota" \
                "Users can add $quota computer accounts to domain" \
    ...
    local eol_count=$(cat "ldap/07_eol_2008.ldif" "ldap/07_eol_win7.ldif" 2>/dev/null | grep -c "^dn:" || echo 0)
    eol_count="${eol_count//[^0-9]/}"
    [ -z "$eol_count" ] && eol_count=0
    
    if [ "$eol_count" -gt 0 ]; then
        add_finding "HIGH" "End-of-Life Systems" \
            "$eol_count systems running EOL operating systems (no security updates)" \
    ...
    pwd_attr_count="${pwd_attr_count//[^0-9]/}"
    [ -z "$pwd_attr_count" ] && pwd_attr_count=0
    
    if [ "$pwd_attr_count" -gt 0 ]; then
        local pwd_attr_users=$(extract_usernames "ldap/08_password_in_attributes.ldif")
        local user_list=$(cat "$pwd_attr_users" | tr '\n' ',' | sed 's/,$//')
        
        add_finding "HIGH" "Credential Exposure" \
            "$pwd_attr_count accounts may have passwords in description/info fields: $user_list" \
