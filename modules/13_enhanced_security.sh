...
check_kerberos_encryption() {
    ...
    while IFS= read -r line; do
        if [[ "$line" == sAMAccountName:* ]]; then
            ...
            # Weak = an explicit value below AES (DES/RC4). We intentionally do
            # NOT flag accounts with no value set: an unset
            # msDS-SupportedEncryptionTypes means "DC default" (AES on modern
            # DCs) and flagging it would produce a false positive for nearly
            # every account.
