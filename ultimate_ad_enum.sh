...
# FIX: Updated add_finding function to handle pipe characters and special chars
add_finding() {
    local level="$1"
    local category="$2"
    local message="$3"
    ...
    # FIX: Escape pipe characters to prevent parsing issues
    # Replace | with Unicode box drawing character │
    message="${message//|/│}"
...
main() {
    ...
    source "$SCRIPT_DIR/modules/13_enhanced_security.sh"
    source "$SCRIPT_DIR/modules/13_enhanced_security.sh"
    source "$SCRIPT_DIR/modules/14_lolbins_persistence.sh"
...
show_final_summary() {
    ...
    if [ $CRITICAL_FINDINGS -gt 0 ] || [ $HIGH_FINDINGS -gt 0 ]; then
        ...
        if [ -f ".findings.tmp" ]; then
            ...
            grep "^CRITICAL" ".findings.tmp" | head -3 | while IFS='|' read -r level category message rest; do
                echo -e "  ${RED}[CRITICAL]${NC} ${category}: ${message:0:70}..."
            done
            grep "^HIGH" ".findings.tmp" | head -2 | while IFS='|' read -r level category message rest; do
